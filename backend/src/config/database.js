...
if (useSQLite) {
  ...
  const initializeTables = async () => {
    ...
    `);
      ...
      // Initialize Sri Lanka Districts Data
      const districts = [
      ...
      console.log('✅ Sri Lanka districts data initialized');

      // Initialize Sample District Paddy Prices (if empty)
      const [priceRows] = await pool.execute('SELECT COUNT(*) as count FROM district_paddy_prices');
      if (priceRows[0].count === 0) {
        const samplePrices = [
          ['Polonnaruwa', 'Nadu - White', 'Dry', 105.00, '2025-01-26'],
          ['Polonnaruwa', 'Samba', 'Dry', 115.00, '2025-01-26'],
          ['Anuradhapura', 'Nadu - Red', 'Dry', 102.50, '2025-01-26'],
          ['Ampara', 'Nadu - White', 'Wet', 92.00, '2025-01-26'],
          ['Kurunegala', 'Samba', 'Dry', 112.00, '2025-01-26']
        ];

        for (const [district, type, condition, price, date] of samplePrices) {
          try {
            await pool.execute(
              'INSERT INTO district_paddy_prices (district_name, paddy_type, paddy_condition, price_per_kg, effective_date) VALUES (?, ?, ?, ?, ?)',
              [district, type, condition, price, date]
            );
          } catch (error) {
            console.error(`Error inserting price for ${district}: `, error.message);
          }
        }
        console.log('✅ Sample district paddy prices initialized');
      }
  ...
  // Initialize tables after a short delay, but only if not running in a script that might close the connection
